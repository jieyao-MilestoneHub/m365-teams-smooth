...
Risk is a qualitative **classification**, not a number. Every fired **evidence tag** — a
machine-readable fact the impact phase derived from reading the affected systems — carries a
severity (`low`/`medium`/`high`), and the trial's level is simply the **most severe** factor that
fired. Nothing is summed and there are no thresholds, so there is no arbitrary number to defend:
each factor is individually accountable for the severity it declares (see
[ADR-0017](adr/0017-severity-classification-over-additive-scoring.md)). Tags come from adapter
reads, deterministic checks, and (strictly within the vocabulary the rule packs define) the
agentic gatherer (see [ADR-0016](adr/0016-llm-deterministic-allocation-contract.md)): a flagged
tag can raise the level or convene an approver, never lower or remove one, and the classification
is always deterministic — the same tags always produce the same level and approvers.

| Level | When | Consequence |
| --- | --- | --- |
| **low** | no factor fired above low severity | No approval required — the requester's confirmation executes the change |
| **medium** | the most severe fired factor is medium | Approval required from the tag-derived roles |
| **high** | a high-severity factor fired | Approval required; a factor that also marks the change unsafe forces a refusal with a safer alternative (in the built-in packs, every high-severity factor is an unsafe outcome) |
...
validated data literal mapping evidence tags to severities and approver roles. The engine
...
| --- | --- | --- | --- |
| `launch_slip` | milestone moves; calendar conflicts; latent contractual/freeze/dependency breaches | eng_lead ← `schedule.milestone_move` · comms ← `comms.pending_announcement` · account_owner ← `schedule.contractual_breach_risk` (policy: any) | a clean coordinated move is medium — feasible, approval required (high is reserved for unsafe outcomes) |
...
| `vendor_access` | folder permission grants with overbroad scope / unclear duration | manager ← `access.overbroad_scope` · security_lead ← `data.customer_data_present` | a scoped, time-boxed grant is low — no risk factor fires |
...
What stays deterministic, on purpose: risk classification, quorum derivation, verdict options,
approval enforcement, and refusal authority. Governance must be reproducible and auditable — that is the
