...
365 tenant needed). Needs Python 3.11+ with [`uv`](https://docs.astral.sh/uv/).
...
`m365/adaptive-cards/generated/` in the [Adaptive Cards Designer](https://adaptivecards.io/designer).
Full walkthrough → [`docs/demo/`](docs/demo/README.md). To serve the API locally:
...
- **Extend it.** A new integration is one adapter implementing `IntegrationAdapter` + one
  registration; a new scenario is one rule pack + gatherer + planner. The graph, services, REST, and
  MCP stay untouched.
